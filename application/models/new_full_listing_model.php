...
class New_full_listing_model extends CI_Model {
	...
	function create_listing(){
		...
		$create_new_listing = array(
			...
			'ad_estado' => $this->input->post('carCondition'),
			'ad_color' => $this->input->post('carColor'),
			'ad_centimetros' => $this->input->post('carCC'),
			'ad_motor' => $this->input->post('carCilinders'),
			'ad_combustible' => $this->input->post('carFuel'),
			'ad_transmision' => $this->input->post('carTrans'),
			'ad_kilometraje' => $this->input->post('carMileage'),
			'ad_puertas' => $this->input->post('carDoors'),
			'ad_postedOn' => $this->input->post('postedOn'),
			'ad_nuevo' => $this->input->post('carPlate'),
			'ad_traccion' => $this->input->post('carDrive'),
			'ad_carroceria' => $this->input->post('carBody'),
			'ad_detalles' => $this->input->post('otherDetails'),
			'ad_complete' => "1"
		);
	...
	function do_upload() {
		...
		for($i = 1; $i <= $this->input->post('totalImages'); $i++){
			$newfilename = $folder_type.$largestid['ad_id']."_".$i.".jpg";
