	...
	<section>
		<div class="centerContainer">
			<div class="mainContentPages">
				<h1>Los anuncios m&aacute;s recientes</h1>
				<?php if($get_recientes): ?>
					...
					<hr />
					<?php foreach($get_recientes as $get_recientes_row): ?>
						<?php 
							$today = date('Y-m-d');
							$posted_date = date_create($get_recientes_row->ad_postedOn);
							...
							$img_folder = strtolower($get_recientes_row->ad_idprefix).$get_recientes_row->ad_id;
							$folder_path = 'cars/small_thumb/'.$img_folder.'/';
							$images = get_filenames($folder_path);
						?>
						<div class="carThumb <?php echo $condition = ($get_recientes_row->ad_nuevo == 1 ? "nuevo": "usado"); ?>">
							<?php if($today >= $posted_date_format && $today <= $week_ahead_format): ?>
									<img src='/images/new_large_badge.png' class='newBadge' width="50" />
								<?php elseif($get_recientes_row->ad_nuevo == "1"): ?>
									<img src='/images/estrenar_badge.png' class='newBadge' width="50" />
								<?php elseif($get_recientes_row->ad_nuevo == "0"): ?>	
									<img src='/images/usado_badge.png' class='newBadge' width="50" />
								<?php endif; ?>
							<?php if($images): ?>
								<a href="/resultados/anuncio_basico/<?php echo $get_recientes_row->ad_idprefix.$get_recientes_row->ad_id; ?>">
							...
							<span class="shortDesc" data-today="<?php echo $today; ?>" data-posted="<?php echo $posted_date_format ?>" data-weekahead="<?php echo $week_ahead_format ?>">
								<?php echo $get_recientes_row->ad_marca ." - ". $get_recientes_row->ad_modelo." ".$get_recientes_row->ad_year; ?><br />
								<?php 
									if ($get_recientes_row->ad_nuevo == "1") {
										echo "Para estrenar";
									}elseif($get_recientes_row->ad_nuevo == "0"){
										echo "Usado"; 
									}
								?><br />
								<?php echo number_format($get_recientes_row->ad_kilometraje, 0, ',', '.')." Km"; ?><br />
								&#8353;
								<?php
									setlocale(LC_MONETARY, 'it_IT');
									echo money_format('%!.0n', $get_recientes_row->ad_precio);
								?><br />
								<a href="/resultados/anuncio_basico/<?php echo $get_recientes_row->ad_idprefix.$get_recientes_row->ad_id; ?>" class="plainDarkLink">Ver detalles</a>
							</span>
						</div>
					<?php endforeach; ?>
					</div>
				<?php elseif(!$get_recientes): ?>
